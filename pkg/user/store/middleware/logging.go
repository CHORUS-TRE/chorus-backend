...
func (c userStorageLogging) RemoveRolesByContext(ctx context.Context, contextDimension, contextValue string) ([]uint64, error) {
	...
	if err != nil {
		c.logger.Error(ctx, logger.LoggerMessageRequestFailed,
			zap.String("contextDimension", contextDimension),
			zap.String("contextValue", contextValue),
			zap.Error(err),
			zap.Float64(logger.LoggerKeyElapsedMs, float64(time.Since(now).Nanoseconds())/1000000.0),
		)
		return nil, err
