...
func (s *AppInstanceStorage) GetAppInstance(ctx context.Context, tenantID uint64, appInstanceID uint64) (*model.AppInstance, error) {
	const query = `
		SELECT id, tenantid, userid, appid, workspaceid, status, createdat, updatedat
			FROM app_instances
...
INSERT INTO app_instances (tenantid, userid, appid, workspaceid, status, createdat, updatedat)
VALUES ($1, $2, $3, $4, $5, NOW(), NOW()) RETURNING id;
	`

	var id uint64
	err := s.db.GetContext(ctx, &id, appInstanceQuery,
		tenantID, appInstance.UserID, appInstance.AppID, appInstance.WorkspaceID, appInstance.Status,
	)
	if err != nil {
		return 0, err
	}
