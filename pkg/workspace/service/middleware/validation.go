...
func (v validation) ListPublicWorkspaces(ctx context.Context, tenantID uint64, pagination *common_model.Pagination, filter model.WorkspaceFilter) ([]*model.PublicWorkspace, *common_model.PaginationResult, error) {
	if err := v.validate.Struct(pagination); err != nil {
		return nil, nil, err
	}
	return v.next.ListPublicWorkspaces(ctx, tenantID, pagination, filter)
}
