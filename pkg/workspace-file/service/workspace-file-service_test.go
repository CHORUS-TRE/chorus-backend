...
func TestFileLifecycle(t *testing.T) {
	...
	// Create file
	createdFile, err := storage.CreateFile(context.Background(), &blockstore.File{
		Path:    storePath,
		Content: content,
	})
...
func TestCreateFileAlreadyExists(t *testing.T) {
	...
	_, err := storage.CreateFile(context.Background(), &blockstore.File{
		Path:    storePath,
		Content: content,
	})
...
func TestDirectoryLifeCycle(t *testing.T) {
	...
	// Create directory
	_, err := storage.CreateDirectory(context.Background(), &blockstore.File{
		Path:        storeDirPath,
		IsDirectory: true,
	})
...
func TestFileUpload(t *testing.T) {
	...
	uploadInfo, err := storage.InitiateMultipartUpload(context.Background(), &blockstore.File{
		Path:        storePath,
		IsDirectory: false,
		Size:        fileSize,
	})
	...
	for partNumber := uint64(1); partNumber <= uploadInfo.TotalParts; partNumber++ {
		...
		part, err := storage.UploadPart(context.Background(), storePath, uploadInfo.UploadID, &blockstore.FilePart{
