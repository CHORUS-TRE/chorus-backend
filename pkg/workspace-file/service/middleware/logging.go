...
func (c workspaceServiceLogging) UpdateWorkspaceFile(ctx context.Context, workspaceID uint64, sourcePath string, file *filestore.File, copy bool) (*filestore.File, error) {
	now := time.Now()

	updatedFile, err := c.next.UpdateWorkspaceFile(ctx, workspaceID, sourcePath, file, copy)
	if err != nil {
		c.logger.Error(ctx, logger.LoggerMessageRequestFailed,
			logger.WithWorkspaceIDField(workspaceID),
			zap.Error(err),
