...
func (s *Caching) ListEntries(ctx context.Context, tenantID uint64, scope model.DevStoreScope, scopeID uint64) ([]*model.DevstoreEntry, error) {
	entry := s.cache.NewEntry(cache.WithUint64(scopeID), cache.WithUint64(scopeID))
	entries := []*model.DevstoreEntry{}

	if ok := entry.Get(ctx, &entries); !ok {
		entries, err := s.next.ListEntries(ctx, tenantID, scope, scopeID)
		if err == nil {
			entry.Set(ctx, defaultCacheExpiration, entries)
		}
	}
	return entries, nil
}

func (s *Caching) GetEntry(ctx context.Context, tenantID uint64, scope model.DevStoreScope, scopeID uint64, key string) (*model.DevstoreEntry, error) {
	return s.next.GetEntry(ctx, tenantID, scope, scopeID, key)
}
