...
if [ -n "$GIT_USERNAME" -a -n "$GIT_PASSWORD" ]; then
    docker build --pull -f dockerfiles/stage2.dockerfile --secret id=GIT_USERNAME,env=GIT_USERNAME --secret id=GIT_PASSWORD,env=GIT_PASSWORD -t harbor.build.chorus-tre.ch/chorus/backend:${IMAGE_TAG} ..
else
