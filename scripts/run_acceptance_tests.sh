...
ACCEPTANCE_CONFIG_SET="${ACCEPTANCE_CONFIG_SET:-storage.datastores.chorus.database=chorus_ci,storage.fileStores.disk.diskConfig.basePath=docker/.diskfilestoreci,clients.docker.enabled=false,clients.kubernetes.enabled=false}"
