// Code generated by go-swagger; DO NOT EDIT.

package devstore_service

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"encoding/json"
	"fmt"
	"io"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"

	"github.com/CHORUS-TRE/chorus-backend/tests/helpers/generated/client/devstore/models"
)

// DevstoreServiceGetUserEntryReader is a Reader for the DevstoreServiceGetUserEntry structure.
type DevstoreServiceGetUserEntryReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *DevstoreServiceGetUserEntryReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewDevstoreServiceGetUserEntryOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	default:
		result := NewDevstoreServiceGetUserEntryDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewDevstoreServiceGetUserEntryOK creates a DevstoreServiceGetUserEntryOK with default headers values
func NewDevstoreServiceGetUserEntryOK() *DevstoreServiceGetUserEntryOK {
	return &DevstoreServiceGetUserEntryOK{}
}

/*
DevstoreServiceGetUserEntryOK describes a response with status code 200, with default header values.

A successful response.
*/
type DevstoreServiceGetUserEntryOK struct {
	Payload *models.ChorusGetEntryReply
}

// IsSuccess returns true when this devstore service get user entry o k response has a 2xx status code
func (o *DevstoreServiceGetUserEntryOK) IsSuccess() bool {
	return true
}

// IsRedirect returns true when this devstore service get user entry o k response has a 3xx status code
func (o *DevstoreServiceGetUserEntryOK) IsRedirect() bool {
	return false
}

// IsClientError returns true when this devstore service get user entry o k response has a 4xx status code
func (o *DevstoreServiceGetUserEntryOK) IsClientError() bool {
	return false
}

// IsServerError returns true when this devstore service get user entry o k response has a 5xx status code
func (o *DevstoreServiceGetUserEntryOK) IsServerError() bool {
	return false
}

// IsCode returns true when this devstore service get user entry o k response a status code equal to that given
func (o *DevstoreServiceGetUserEntryOK) IsCode(code int) bool {
	return code == 200
}

// Code gets the status code for the devstore service get user entry o k response
func (o *DevstoreServiceGetUserEntryOK) Code() int {
	return 200
}

func (o *DevstoreServiceGetUserEntryOK) Error() string {
	payload, _ := json.Marshal(o.Payload)
	return fmt.Sprintf("[GET /api/rest/v1/devstore/user/{key}][%d] devstoreServiceGetUserEntryOK %s", 200, payload)
}

func (o *DevstoreServiceGetUserEntryOK) String() string {
	payload, _ := json.Marshal(o.Payload)
	return fmt.Sprintf("[GET /api/rest/v1/devstore/user/{key}][%d] devstoreServiceGetUserEntryOK %s", 200, payload)
}

func (o *DevstoreServiceGetUserEntryOK) GetPayload() *models.ChorusGetEntryReply {
	return o.Payload
}

func (o *DevstoreServiceGetUserEntryOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.ChorusGetEntryReply)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewDevstoreServiceGetUserEntryDefault creates a DevstoreServiceGetUserEntryDefault with default headers values
func NewDevstoreServiceGetUserEntryDefault(code int) *DevstoreServiceGetUserEntryDefault {
	return &DevstoreServiceGetUserEntryDefault{
		_statusCode: code,
	}
}

/*
DevstoreServiceGetUserEntryDefault describes a response with status code -1, with default header values.

An unexpected error response.
*/
type DevstoreServiceGetUserEntryDefault struct {
	_statusCode int

	Payload *models.RPCStatus
}

// IsSuccess returns true when this devstore service get user entry default response has a 2xx status code
func (o *DevstoreServiceGetUserEntryDefault) IsSuccess() bool {
	return o._statusCode/100 == 2
}

// IsRedirect returns true when this devstore service get user entry default response has a 3xx status code
func (o *DevstoreServiceGetUserEntryDefault) IsRedirect() bool {
	return o._statusCode/100 == 3
}

// IsClientError returns true when this devstore service get user entry default response has a 4xx status code
func (o *DevstoreServiceGetUserEntryDefault) IsClientError() bool {
	return o._statusCode/100 == 4
}

// IsServerError returns true when this devstore service get user entry default response has a 5xx status code
func (o *DevstoreServiceGetUserEntryDefault) IsServerError() bool {
	return o._statusCode/100 == 5
}

// IsCode returns true when this devstore service get user entry default response a status code equal to that given
func (o *DevstoreServiceGetUserEntryDefault) IsCode(code int) bool {
	return o._statusCode == code
}

// Code gets the status code for the devstore service get user entry default response
func (o *DevstoreServiceGetUserEntryDefault) Code() int {
	return o._statusCode
}

func (o *DevstoreServiceGetUserEntryDefault) Error() string {
	payload, _ := json.Marshal(o.Payload)
	return fmt.Sprintf("[GET /api/rest/v1/devstore/user/{key}][%d] DevstoreService_GetUserEntry default %s", o._statusCode, payload)
}

func (o *DevstoreServiceGetUserEntryDefault) String() string {
	payload, _ := json.Marshal(o.Payload)
	return fmt.Sprintf("[GET /api/rest/v1/devstore/user/{key}][%d] DevstoreService_GetUserEntry default %s", o._statusCode, payload)
}

func (o *DevstoreServiceGetUserEntryDefault) GetPayload() *models.RPCStatus {
	return o.Payload
}

func (o *DevstoreServiceGetUserEntryDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.RPCStatus)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
