		...
		Given("a valid jwt-token", func() {
			...
			When("the route GET '/api/rest/v1/users' is called", func() {
				...
				Then("users should be returned with email", func() {
					setupTables()
					auth := getAuthAsClientOpts(helpers.CreateJWTToken(1, 88888, authorization.SuperAdmin.Name.String(), map[string]string{"user": "*"}))
					req := user.NewUserServiceListUsersParams()
		...
		Given("a valid jwt-token", func() {
			...
			Given("an unauthorized role", func() {
				...
				When("the route GET '/api/rest/v1/users/{id}' is called", func() {
					...
					Then("a permission error should be raised", func() {
						auth := getAuthAsClientOpts(helpers.CreateJWTToken(1, 88888, authorization.Authenticated.Name.String(), map[string]string{"user": "1"}))
						req := user.NewUserServiceGetUserParams().WithID("90000")
		...
		Given("a valid jwt-token", func() {
			...
			When("the route GET '/api/rest/v1/users/{id}' is called", func() {
				...
				Then("a user should be returned", func() {
					setupTables()
					auth := getAuthAsClientOpts(helpers.CreateJWTToken(90000, 88888, authorization.SuperAdmin.Name.String(), map[string]string{"user": "90000"}))
					req := user.NewUserServiceGetUserParams().WithID("90000")
	...
	Describe("get me", func() {
		...
		Given("a valid jwt-token", func() {
			...
			When("the route GET '/api/rest/v1/users/me' is called", func() {
				...
				Then("a user should be returned", func() {
					setupTables()
					auth := getAuthAsClientOpts(helpers.CreateJWTToken(90000, 88888, authorization.Authenticated.Name.String(), map[string]string{"user": "90000"}))
		...
		Given("a valid jwt-token", func() {
			...
			Given("an unauthorized role", func() {
				...
				When("the route POST '/api/rest/v1/users/me/totp/enable' is called", func() {
					...
					Then("a permission error should be raised", func() {
						auth := getAuthAsClientOpts(helpers.CreateJWTToken(1, 88888, authorization.Public.Name.String(), map[string]string{"user": "1"}))
						req := user.NewUserServiceEnableTotpParams().WithBody(
							&models.ChorusEnableTotpRequest{
								Totp: "totp",
							},
			...
			Given("an empty field in request", func() {
				...
				When("the route POST '/api/rest/v1/users' is called", func() {
					...
					Then("a validation error should be raised", func() {
						auth := getAuthAsClientOpts(helpers.CreateJWTToken(90000, 88888, string(authorization.PlatformUserManager.Name), map[string]string{}))
