...
func workspaceAdminAuth() func(*runtime.ClientOperation) {
	return getAuthAsClientOpts(helpers.CreateJWTToken(90000, 88888, authorization.RoleWorkspaceAdmin.Name.String(), map[string]string{"workspace": "80001"}))
}

func authenticatedAuth(userID uint64) func(*runtime.ClientOperation) {
	return getAuthAsClientOpts(helpers.CreateJWTToken(userID, 88888, authorization.RoleAuthenticated.Name.String(), map[string]string{"user": fmt.Sprintf("%d", userID)}))
}

func platformWorkspaceManagerAuth(userID uint64) func(*runtime.ClientOperation) {
	return getAuthAsClientOpts(helpers.CreateJWTToken(userID, 88888, authorization.RolePlatformWorkspaceManager.Name.String(), map[string]string{"workspace": "*"}))
}
		...
		Given("a valid jwt-token with no access to the workspace", func() {
			When("GET /api/rest/v1/workspaces/{id} is called", func() {
				...
				Then("a permission error is returned", func() {
					auth := getAuthAsClientOpts(helpers.CreateJWTToken(90001, 88888, authorization.RoleAuthenticated.Name.String(), map[string]string{"user": "90001"}))
					req := workspace_svc.NewWorkspaceServiceGetWorkspaceParams().WithID("80001")
					c := helpers.WorkspaceServiceHTTPClient()
