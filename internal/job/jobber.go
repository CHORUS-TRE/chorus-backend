...
func NewJobber(lockStore LockStore, checkInterval time.Duration, jitter float64, log *logger.ContextLogger) *Jobber {
	return &Jobber{
...
func (j *Jobber) runJob(ctx context.Context, name string, reg Registration) {
	...
	if err := j.lockStore.Release(ctx, name, j.ownerID); err != nil {
