...
func runServer() error {
	...
	handler, mux, opts := rest.InitServer(ctx, cfg, getVersion(), started, provider.ProvideWorkbench().ProxyWorkbench, provider.ProvideAuthorizer(), provider.ProvideWorkbench(), provider.ProvideKeyFunc(cfg.Daemon.JWT.Secret.PlainText()), provider.ProvideClaimsFactory(), provider.ProvideOIDCIDPService())
...
func registerHTTPEndpoints(ctx context.Context, mux *runtime.ServeMux, grpcHostPort string, opts []google_grpc.DialOption) {
	...
	if err := chorus.RegisterPlatformSettingsServiceHandlerFromEndpoint(ctx, mux, grpcHostPort, opts); err != nil {
		logger.TechLog.Fatal(ctx, "failed to register http platform settings service handler", logger.WithErrorField(err))
	}
