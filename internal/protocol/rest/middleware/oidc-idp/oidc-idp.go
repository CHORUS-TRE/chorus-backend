...
import (
	...
	"github.com/CHORUS-TRE/chorus-backend/internal/protocol/rest/middleware/storage"
	"github.com/luikyv/go-oidc/pkg/goidc"
	"github.com/luikyv/go-oidc/pkg/provider"
	"go.uber.org/zap"
)

func AddOIDC(h http.Handler, cfg config.Config) http.Handler {
	if !cfg.Services.OpenIDConnectProvider.Enabled {
		return h
	}

	authnSessionStorage := storage.NewAuthnSessionManager(1000)
	clientStorage, err := storage.NewClientManager(cfg)
	if err != nil {
	...
	op, err := provider.New(
		...
		provider.WithPolicies(authutil.Policy(cfg)),
		...
		provider.WithAuthnSessionStorage(authnSessionStorage),
		provider.WithClientStorage(clientStorage),
		provider.WithGrantSessionStorage(grantSessionStorage),
		provider.WithLogoutSessionManager(logoutSessionStorage),
	)
	if err != nil {
		logger.TechLog.Fatal(context.Background(), "unable to instantiate an OIDC provider", zap.Error(err))
	}

	openidIDPHandler := op.Handler()
