...
echo ""
echo "deploying..."
helm upgrade --install --create-namespace --namespace "$env" --values ./backend/files/values.yaml --set-string "aesPassphrase=${!CONFIG_AES_PASSPHRASE_VAR_NAME}" --set-string "version=$RELEASE_NAME" --set-string "image=registry.dip-dev.thehip.app/chorus-cicd-chorus:${IMAGE_TAG}" "${RELEASE_NAME}" ./backend
echo "done"